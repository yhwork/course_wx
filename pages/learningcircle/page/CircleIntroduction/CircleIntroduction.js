...
var time = 600;
var timer;
class CircleIntroductionPage extends EPage {
  get data() {
    return {
      userInfo: {},
      arr: [{
  ...
  mapUIEvent({
    ...
    return {
      ...
      [events.ui.CHANGE_AVATAR](e) {
        ...
        if (type == 2) {
          //判断圈子介绍图片数量,大于3时不让上传
          let contentImgNm = parseInt(this.data.contentImgNm);
          if (contentImgNm >= 5) {
            this.setData({
              toast3Hidden: false
            });
                ...
                this.$api.upload.upload(resp.tempFilePaths[0]).then(res => {
                  if (type == '0') {
                    ...
                    this.setData({
                      ...
                      contentImgNm: parseInt(this.data.contentImgNm) + 1
                    })
                  }
                });
              }
      ...
      [events.ui.DOWN](e) {
        ...
        if (type == 1) {
          this.setData({
            ...
            communityIntroduceForPage: Array.downIndex(array, idx)
          })
          console.log(this.data.communityIntroduce);
        } 
      },
  ...
  mapEffect() {
    return {
      ...
      [effects.GENERAGE_CODE]() {
        this.$api.user.generateCode().then(
          (res) => {
            if (res.data.errorCode == '0') {
              this.setData({
                shareCodeUni: res.data.result
              })
            } else {
              this.setData({
                shareCodeUni: ''
      ...
      [effects.SAVE_NEW_CIRCLE_INFO]() {
        ...
        this.$api.circle.updateCommunity(this.data.model).then((res) => {
          ...
          if (this.data.Jump){
           wx.redirectTo({
             url: '../circleEditor/circleEditor?id=' + this.data.communityId + '&Jump=' + this.data.Jump,
           })
         }else(
            wx.redirectTo({
              url: '../circleEditor/circleEditor?id=' + this.data.communityId 
            })
         )
...
EApp.instance.register({
  type: CircleIntroductionPage,
  id: 'CircleIntroductionrPage',
  config: {
    events,
    effects,
    actions
