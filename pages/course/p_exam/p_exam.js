    ...
    return {
      ...
      [PAGE_LIFE.ON_SHARE_APP_MESSAGE](option) {
        ...
        return {
          ...
          imageUrl: "/pages/assets/icons/examimg.jpg",
  ...
  mapUIEvent({ //页面事件方法  
    ...
    return {
      [events.ui.radioChange](e) {
        ...
        console.log("打印  点击radio的数据  ====   ")
        console.log(this.data.itemsA[num].valuesArray[index].clicktime)
  ...
  mapEffect() { //调接口 方法   存储方法方便调用
    return {
      [effects.questionnaire]() {
        this.$api.circle.questionnaire({ contents }).then((res) => {
      ...
      [effects.shareInfoRecord]() {
        this.$api.circle.shareInfoRecord({ dataType: 8, data: {} }).then((res) => {
          console.log("分享分享=======")
