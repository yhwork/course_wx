...
import { events, effects, actions } from './calendarDiary.eea'
import moment from '../../lib/moment.min.js';
class CalendarDiaryComponent extends EComponent {
  get properties() {
    return {
      diaryInfo: {
          type: Object,
          value: {},
          observer: (value) => {
              this.context.dispatch(actions.MAP_DIARY_DATA,value);
          }
      },
      /**
       * 年份
  ...
  mapAction({ publish, put, dispatch }) {
    return {
      [actions.MAP_DIARY_DATA](data) {
          //console.log(data);
          if(data==null){return}
          let communityStartDate = '';
          let communityEndDate = '';
          ...
          days_array1.forEach(
              (item1)=>{
                  item1.forEach(
                     (item2)=>{
                        if(typeof item2.diaryStatus!='undefined'){
                          delete item2.diaryStatus
                        }
                     } 
                  )
              }
          ...
          if (!data || data.signRecordList.length === 0) { return; }
          days_array1 = this.data.days_array;
          data.signRecordList.forEach(
              (item)=>{
                  days_array1.forEach(
                      (item1)=>{
                          item1.forEach(
                             (item2)=>{
                                ...
                                let currentDate = moment(item2.year+'-'+item2.month+'-'+item2.day,'YYYY-MM-DD').format('YYYY-MM-DD')
                                if( moment(currentDate).isBetween(moment(data.communityStartDate).add(-1,'days'), moment(data.communityEndDate).add(1,'days')) && moment(currentDate).isBefore(moment().format('YYYY-MM-DD'))){
                                    item2.durationDate = 1;
                                }
                             } 
                          )
                      }
...
EApp.instance.registerComponent({
  type: CalendarDiaryComponent,
  id: 'CalendarDiaryComponent',
