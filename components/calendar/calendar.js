...
import { events, effects, actions } from './calendar.eea'
import moment from '../../lib/moment.min.js';
class CalendarComponent extends EComponent {
  get properties() {
    return {
      calendarClass: {
          type: String,
      ...
      [actions.MAP_LESSON_DATA](data) {
          ...
          days_array1.forEach(
              (item1)=>{
                  item1.forEach(
                     (item2)=>{
                        if(typeof item2.courseStatus!='undefined'){
                          delete item2.courseStatus
                        }
                     } 
                  )
              }
                          ...
                          item1.forEach(
                             (item2)=>{
                                if(item2.year==item.year && item2.month==item.month && item2.day==item.day){
                                  item2.courseStatus = item.courseStatus;
                                }
                             } 
                          )
                      }
